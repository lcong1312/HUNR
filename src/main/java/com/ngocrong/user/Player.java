...
//                            }
    ...
    public void chatPlayer(Player _player, String text) {
        if (false && this.session.user.getActivated() == 0) {
...
//        }
        ...
        if (mapID >= 156 && mapID <= 158) {
            return info.power > 40_000_000_000L;
        }
    ...
    public void chatGlobal(Message ms) {
        if (!requireActivatedForWorldChat()) {
            service.sendThongBao("Bạn cần kích hoạt thành viên để sử dụng tính năng này");
            return;
        }
    ...
    public boolean isActivatedMember() {
        return this.session != null && this.session.user != null && this.session.user.isActivatedMember();
    }

    private boolean hasCompletedMembershipActivationTask() {
        return this.taskMain != null && (this.taskMain.id > 20 || (this.taskMain.id == 20 && this.taskMain.index >= 6));
    }

    public void syncActivatedMembershipStatus() {
        syncActivatedMembershipStatus(false);
    }

    public void syncActivatedMembershipStatus(boolean notify) {
        if (this.session == null || this.session.user == null || this.isActivatedMember()
                || !this.hasCompletedMembershipActivationTask()) {
            return;
        }
        this.session.user.activateMembership();
        if (notify && this.service != null) {
            this.service.sendThongBao("Tai khoan da duoc kich hoat thanh vien.");
        }
    }

    private boolean requireActivatedForWorldChat() {
        return this.isActivatedMember();
    }

    private boolean requireActivatedForTrade() {
        if (this.isActivatedMember()) {
            return true;
        }
        this.service.sendThongBao("Tai khoan can kich hoat de co the giao dich");
        return false;
    }
    ...
    public void taskNext() {
        ...
        syncActivatedMembershipStatus(true);
...
//                        menus.add(new KeyValue(1138, "Hồi Skill Ngay\n(200tr Vàng)"));
                    ...
                    case NpcName.CUA_HANG_KY_GUI:
                        // khoidt
                        if (false && this.session.user.getActivated() != 1) {
...
//                break;
            ...
            case 1104:
                ...
                if (next == 126 && Utils.nowVietnamDateTime().getHour() != 22) {
            ...
            if (this.zone != null) {
                try {
                    ...
                    if (!z.getBossInZone().isEmpty()) {
                        ...
                        if (false && this.session.user.getActivated() == 0 && !flag) {
...
//        }
    ...
    public void joinMap(int mapid, boolean checkMTV) {
        if (false && this.getSession().user.getActivated() == 0 && checkMTV) {
            service.sendThongBao("Bạn cần kích hoạt tài khoản để có thể vào map này");
            return;
        }
        var map3 = MapManager.getInstance().getMap(mapid);
