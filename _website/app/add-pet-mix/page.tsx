...
export default function AddPetMixPage() {
  ...
  const handleClassifyByParts = async () => {
    ...
    if (!headIds.length && !bodyIds.length && !legIds.length) {
      setPopup({ message: 'Nhập part cũ (head/body/leg) trước', type: 'error' })
      return
    }
    ...
    try {
      ...
      if (matched === 0) {
        ...
        setPopup({ message: `Đã phân loại ${matched} ảnh theo part cũ`, type: 'success' })
      }
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await response.json()
      if (!response.ok) throw new Error(data.error || 'Thêm pet thất bại')
      setPopup({ message: data.message, type: 'success' })
      setFormData({
        name: '',
        description: '',
      ...
      const msg = err instanceof Error ? err.message : 'Thêm pet thất bại'
      setPopup({ message: msg, type: 'error' })
    } finally {
      setSubmitting(false)
    }
        ...
        <div className="max-w-6xl mx-auto space-y-6">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Pet Mix</h1>
              <p className="text-sm text-gray-600">Upload ảnh mới → chuyển part → thêm pet</p>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 md:p-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-500">Head / Body / Leg (nhiều ảnh) + Icon (lẻ)</p>
              </div>
            </div>
          ...
          <div className={`bg-white rounded-xl shadow-lg p-6 md:p-8 ${step < 2 ? 'opacity-60 pointer-events-none' : ''}`}>
            <div className="flex items-center gap-3 mb-6">
              ...
              <div className="flex-1">
                <h2 className="text-xl md:text-2xl font-bold text-gray-900">Bước 3: Thêm pet</h2>
                <p className="text-sm text-gray-500">Điền thông tin pet. Có thể dán part mới vào Head/Body/Leg.</p>
              </div>
              <div className="flex gap-2">
